class El {
    ...
    set classes (value) {
        if (value != '') {
            (Array.isArray(value)) ? this.#currentClasses = value : this.#currentClasses = value.split(' ');
            this.element.classList.remove(...this.element.classList);
            this.#currentClasses.forEach(className => {
            this.element.classList.add(className);
        })
        } else { // Removes all classes
            this.#currentClasses = []
            this.element.classList.remove(...this.element.classList);
        }
    }
    ...
    set addClass (value) { // Adds single class
    ...
    set parent(parentIdentifier) {
        (typeof parentIdentifier == 'string') ? this._parent = document.querySelector(parentIdentifier) : this._parent = parentIdentifier;
